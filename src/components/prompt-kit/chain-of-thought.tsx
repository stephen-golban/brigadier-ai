...
export const ChainOfThoughtTrigger = ({
  ...
  <CollapsibleTrigger
    className={cn(
      "group [&[data-state=open]_[data-chevron]]:rotate-180 text-muted-foreground hover:text-foreground flex cursor-pointer items-center justify-start gap-1 text-left text-sm transition-colors",
      className,
    )}
    {...props}
  >
    ...
    {!leftIcon && (
      <ChevronDown data-chevron className="size-4 transition-transform " />
    )}
  </CollapsibleTrigger>
);
