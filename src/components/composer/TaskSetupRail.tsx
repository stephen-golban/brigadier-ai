            ...
            {existing ? <>
              ...
              <div role="listbox" aria-label="Existing worktree" onKeyDown={navigateItems}>
                {options?.worktrees.filter(tree => `${tree.path} ${tree.branch ?? ""} ${tree.activeTaskTitle ?? ""}`.toLowerCase().includes(query.toLowerCase())).map(tree => <button key={tree.path} type="button" role="option" aria-label={`${tree.branch ?? "Detached HEAD"} ${tree.path}${tree.activeTaskTitle ? ` In use by ${tree.activeTaskTitle}` : tree.reason ? ` ${tree.reason}` : ""}`} disabled={!tree.available} aria-selected={picks.workspacePath === tree.path} className="setup-menu-option" onClick={() => chooseEnvironment(false, tree.path)}>
                  <BranchAlt /><span>{tree.branch ?? "Detached HEAD"}<small>{" "}{tree.path}</small>{!tree.available && <small>{" "}{tree.activeTaskTitle ? `In use by ${tree.activeTaskTitle}` : tree.reason ?? "Unavailable"}</small>}</span>{picks.workspacePath === tree.path && <Check />}
                </button>)}
              ...
              <button type="button" role="option" aria-selected={picks.isolated && !picks.workspacePath} disabled={!options?.isGit} className="setup-menu-option" onClick={() => chooseEnvironment(true)}><BranchAlt /><span>New worktree{!options?.isGit && <small>This folder is not a Git repository.</small>}</span>{picks.isolated && !picks.workspacePath && <Check />}</button>
              <button type="button" role="option" aria-selected={!!picks.workspacePath} disabled={!options?.worktrees.length} className="setup-menu-option" onClick={() => setExisting(true)}><BranchAlt /><span>Existing worktree{!options?.worktrees.length && <small>No worktrees in this project yet.</small>}</span>{picks.workspacePath && <Check />}</button>
            </div>}
          </Popover.Dialog>
        </Popover.Content>
      </Popover>
