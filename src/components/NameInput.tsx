...
function moveCaret(input: HTMLInputElement, key: string, shift = false, word = false, edge = false) {
  ...
  else if (key === "End" || key === "ArrowDown" || edge) caret = input.value.length;
...
type Props = Omit<ComponentProps<typeof Input>, "value" | "onChange" | "onKeyDown" | "type"> & {
  value: string;
  ...
  return (
    <Input
      ...
      onKeyDown={(event) => {
        ...
        moveCaret(event.currentTarget, key, event.shiftKey, event.altKey || event.ctrlKey, event.metaKey);
      }}
      onChange={(event) => {
        const input = event.currentTarget;
        const raw = input.value;
        const start = filterName(raw.slice(0, input.selectionStart ?? raw.length)).length;
        const end = filterName(raw.slice(0, input.selectionEnd ?? raw.length)).length;
        ...
        if (raw !== clean) {
          ...
          const arrow = native.inputType === "insertText" && native.data ? arrows[native.data] : undefined;
