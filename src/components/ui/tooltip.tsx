...
function TooltipContent({
  ...
  > & {
    /**
     * **Deviation from upstream** (`src/components/ui/UPSTREAM.md`). Upstream always renders
     * `TooltipPrimitive.Arrow`, and its arrow is `bg-foreground` — a filled square rotated 45°,
     * which only disappears into the popup because upstream's popup is `bg-foreground` too.
     * brigadier's tooltip panel is `glass-surface bg-elevated` (`controls/tooltip.tsx`), so the
     * arrow renders as a white diamond hanging off the panel. Opt out rather than restyle it.
     */
    showArrow?: boolean;
  }) {
  return (
