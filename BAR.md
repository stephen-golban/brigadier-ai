...
| 80 a fourth audience: a human in a session a model also reads | item 11 — the cap and its one-way property are unchanged; the fourth state and D24's one-line form are what this ruling adds. `watched-session` now exists in the enum and is CAPPED with `host-session`: a human watching does not make the model's window cheaper. Its in-flight shape is `chunked`, which ruling 80 names as UNMEASURED in the ruling that adopts it |
| 81 outcomes may feed competence; cost still may not | item 13, and `bun run claims` check 4 — the router↛cost seam stays UNCHANGED and the outcome store gains a second entry of its own, so this ruling moves the gate by ADDING to it. `test/forbidden-imports.test.ts` drives the hop the new entry exists for (outcome store → cost → ranking) and the move it survives (the store leaving `src/router/`). The exploration floor is mandatory in the mechanism and its size is the operator's, printed beside every ranking it protects |
...
| 88 failover is reactive and says so; the review decision clamps upward | items 5 and 6 (a reviewer runs cross-vendor where one exists; a single-vendor machine degrades visibly). **D9's rule has the load-bearing test and it is a NEGATIVE**: an unclassified failure marks nothing cold, because a mechanism that routed away on every failure would pass every "does failover work" check while walking broken work down the whole fleet. The clamp's one-way property is swept exhaustively rather than exampled. **What no item covers: whether the cold windows are the right length** — they are judgements, printed beside every exclusion they cause, and nothing here measures them. Ruled under delegated authority |
| 87 items spread across vendors; the router is no longer `agents[0]` | items 4 and 6, and `test/assign.test.ts`, whose load-bearing assertion is the NEGATIVE CONTROL: not "each item got an agent" — which the defective code satisfied — but that five items on three vendors are not five workers on one. **What no item covers is whether the ranking is any good**: `src/router/table.ts`'s own header says *"not one row says `measured`"*, and ruling 81's learned column is evidence about one machine. This row grades the SPREAD, and says so |
| 86 D24's line form is a constructor that refuses, not a lint | item 11 (the report fits the window the owner is working in — unchanged) and `test/say.test.ts`, whose load-bearing assertion is the REFUSAL: a `say()` that flattened would satisfy every "is it one line" check while keeping the paragraph, which is the failure a word list has. **Tone has no item and is not linted** — D24 declines one, the standard is in `AGENTS.md`, and it is enforced by review, which is a person. Ruled under delegated authority |
| 85 brigadier asks before the spend you did not ask for | **item 15 — NOT YET BUILT** (assertion 6: a question round-trips through exit-and-resume). `test/cli-run.test.ts` drives the round trip at the process boundary, including the NEGATIVE CONTROL that matters most — an ordinary goal does NOT stop, because a question asked every time is wallpaper and its answer stops being read (ruling 59's shape). D15's expiry is covered in `test/pending.test.ts` with a survival beside every expiry |
