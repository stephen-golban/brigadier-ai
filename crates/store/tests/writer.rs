...
use brigadier_store::{SessionRow, Store, StoreConfig};
...
async fn ten_thousand_feed_ops_are_fast_and_the_ring_holds() {
    ...
    for seq in 0..10_000u64 {
        ...
        store
            .handle()
            .feed(id, seq, SystemTime::now(), format!("row {seq}"))
            .await
            .expect("feed");
    }
    store.handle().flush().await.expect("flush");
...
async fn an_op_sent_before_flush_is_visible_after_it() {
    ...
    store
        .handle()
        .feed(id.clone(), 1, SystemTime::now(), "hello".into())
        .await
        .expect("feed");
    store.handle().flush().await.expect("flush");
...
async fn fifty_thousand_feed_ops_do_not_grow_the_file() {
    ...
    for seq in 0..50_000u64 {
        store
            .handle()
            .feed(id.clone(), seq, SystemTime::now(), format!("{seq} {payload}"))
            .await
            .expect("feed");
    }
    store.handle().flush().await.expect("flush");
...
async fn deleting_a_project_cascades_to_its_sessions_and_their_children() {
    ...
    store.handle().feed(id.clone(), 1, SystemTime::now(), "x".into()).await.expect("feed");
    store.handle().flush().await.expect("flush");
...
async fn a_query_does_not_wait_out_the_batch_window() {
    ...
    for seq in 0..5u64 {
        store
            .handle()
            .feed(id.clone(), seq, SystemTime::now(), format!("row {seq}"))
            .await
            .expect("feed");
    }
...
async fn a_lone_op_commits_on_its_own_window_and_the_writer_then_parks() {
    ...
    let store = open_store(dir.path(), 500);
    let id = SessionId::new("s1");
    store.handle().feed(id.clone(), 1, SystemTime::now(), "lone".into()).await.expect("feed");
...
async fn a_flush_after_shutdown_fails_instead_of_hanging() {
    ...
    store
        .handle()
        .feed(SessionId::new("s1"), 1, SystemTime::now(), "x".into())
        .await
        .expect("feed");
    store.close().await.expect("close");
