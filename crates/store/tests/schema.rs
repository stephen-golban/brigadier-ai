...
async fn open_migrates_and_sets_the_pragmas_that_cannot_be_retrofitted() {
    ...
    // 8 == migration 0 (the tables), migration 1 (`feed.kind`, added 2026-09-03; edited in place
    ...
    // Migration 7 adds rewind journals and provider UUIDs.
    // Migration 6 adds durable chat items (2026-09-05). Every default is proved in `schema::tests`.
    assert_eq!(h.pragma_i64("user_version").await.expect("user_version"), 8);
    ...
    assert_eq!(h.pragma_i64("journal_size_limit").await.expect("limit"), 64 * 1024 * 1024);
    ...
    assert!(h.pragma_i64("Journal Mode").await.is_err(), "pragma names are not interpolated");
...
async fn reopening_is_idempotent_and_mints_a_new_run_id() {
    ...
    assert_eq!(first.handle().pragma_i64("user_version").await.expect("v"), 8);
    first.close().await.expect("close");

    let second = Store::open_with(dir.path(), StoreConfig::default()).expect("reopen");
    assert_eq!(second.handle().pragma_i64("user_version").await.expect("v"), 8);
