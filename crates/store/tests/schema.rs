...
async fn open_migrates_and_sets_the_pragmas_that_cannot_be_retrofitted() {
    ...
    // 6 == migration 0 (the tables), migration 1 (`feed.kind`, added 2026-09-03; edited in place
    ...
    // Every default is proved in `schema::tests`.
    assert_eq!(h.pragma_i64("user_version").await.expect("user_version"), 6);
...
async fn reopening_is_idempotent_and_mints_a_new_run_id() {
    ...
    assert_eq!(first.handle().pragma_i64("user_version").await.expect("v"), 6);
    first.close().await.expect("close");

    let second = Store::open_with(dir.path(), StoreConfig::default()).expect("reopen");
    assert_eq!(second.handle().pragma_i64("user_version").await.expect("v"), 6);
