...
with (out/f'{label}-run.log').open('w') as log:
 ...
 if len(fresh)==1 and child.poll() is None:
  pid=next(iter(fresh));evidence['sampleExit']=subprocess.run(['/usr/bin/sample',str(pid),'15','1','-file',str(out/f'{label}-sample.txt')],stdout=log,stderr=log).returncode
 else:evidence['error']='No unique new WebContent process; no sample taken'
