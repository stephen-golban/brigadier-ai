...
fn terminal(status: &str) -> bool {
    matches!(
        status,
        "Idle" | "Needs approval" | "Rewinding" | "exited" | "failed"
    )
}
...
async fn wait(ready: &Ready, caller: &str, v: &Value) -> Result<Value, AppError> {
    ...
    let reason = loop {
        ...
        for target in &targets {
            match summary(ready, target, false).await {
...
pub(crate) async fn dispatch(ready: &Ready, caller: &str, v: &Value) -> Result<Value, AppError> {
    match v["action"].as_str() {
        ...
        Some("list") => {
            ...
            let sessions = rows.iter().filter(|s| project.is_none_or(|p| s.project_id.as_deref() == Some(p)) && require_target(ready, s).is_ok()).take(200).map(|s| json!({"id":s.session_id,"projectId":s.project_id,"model":s.model,"status":s.status.as_str(),"live":ready.supervisor.is_live(&s.session_id),"cwd":s.cwd,"startedBy":data.origins.get(s.session_id.as_str()),"title":data.titles.get(s.session_id.as_str())})).collect::<Vec<_>>();
