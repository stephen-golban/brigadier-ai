...
rm -f "$W/out-fix/burn-$LABEL.json"
LOCKED=$(python3 -c "import subprocess,plistlib;print(plistlib.loads(subprocess.check_output(['ioreg','-n','Root','-d1','-a'])).get('IOConsoleLocked'))")
...
python3 scripts/measure-native-burn.py "$W/frozen-$ARM.app/Contents/MacOS/brigadier" \
  ...
  --data-dir "$DATA" \
  --output "$W/out-fix/burn-$LABEL.json" > "$W/out-fix/burn-$LABEL.stdout" 2>&1
CODE=$?
...
echo "burn_label=$LABEL arm=$ARM exit=$CODE locked_before=$LOCKED pgrep_before=$PG_BEFORE pgrep_after=$PG_AFTER load_before=$LOAD_BEFORE load_after=$LOAD_AFTER locks_before=$LOCKS_BEFORE locks_after=$LOCKS_AFTER" > "$W/out-fix/burn-$LABEL.exit"
cat "$W/out-fix/burn-$LABEL.exit"
exit $CODE
